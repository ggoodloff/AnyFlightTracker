# screenshot.py
#
# goodloffg@gmail.com - modified OverPutney dwtached fork to use facebook multiimage post
#
...
isolate = parser.getboolean('abovetustin', 'isolate_enable')
#show_site = parser.getboolean('abovetustin', 'show_site')
map_params_screenshot_isolate = parser.get('abovetustin', 'map_params_screenshot_isolate')
map_params_screenshot = parser.get('abovetustin', 'map_params_screenshot')
#site_lat =  parser.get('receiver', 'latitude')
#site_lon = parser.get('receiver', 'longitude')
sleep_time = int(parser.get('abovetustin', 'sleeptime'))
request_timeout = int(parser.get('abovetustin', 'requesttimeout'))
...
options.add_argument("--user-data-dir=/home/tracker/AnyFlightTracker/chromium") # <- Change this to chromium cache dir that has 777 permissions
...
####################              Updated 2/25/2023                  ########################
    ...
    def clickOnAirplane(self, hex):
            ...
            try:
#                if isolate:
#                   url = f"{self.url}?icao={hex}{map_params_screenshot_isolate}"
#                else:
#                    url = f"{self.url}?icao={hex}{map_params_screenshot}"
                url = f"{self.url}?icao={hex}"
                url += datasource.g_map_parameters

                print(f"Opening {url}")
#                my_logger.info(f"Opening {url}")
...
#                    my_logger.exception('Exception: %s' % err)

                if self.browser:
                    # if we don't wait a little bit the airplane icon isn't drawn.
                    time.sleep(sleep_time)

#                return self.screenshot(hex)
                    self.screenshot('screen.png') # <- Image_3 in config.ini
                    browser.close()

                else:
                    print('Could not find browser')
#                    my_logger.error('Could not find browser')
                    return None
...
#        browser.set_window_size(abovetustin_image_width, abovetustin_image_height)
    ...
    def clickOnAirplane(self, text):
        ...
        try:
            ...
            aircraft.click()
            time.sleep(7.5) # if we don't wait a little bit the airplane icon isn't drawn.
            show_on_map = self.browser.find_element_by_link_text('Show on map')
            show_on_map.click()
            time.sleep(1.0)
            return self.screenshot('tweet.png') #twitter image file location <- saves in program directory
